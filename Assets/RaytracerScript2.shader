Shader "Unlit/SingleColor"
{
	Properties
	{
		...
		_FocusDistance("Focus Distance", Float) = 9
	}
		SubShader{ Pass{
		CGPROGRAM
...
#pragma fragment frag
	...
	//emulate checker texture as a function of 2 colors
	vec3 checker_texture(in vec3 p, in vec3 o, in vec3 e) {
	...
	//material type = int | 0 = lambert | 1 = metal | 2 = dielectric | 
	class material {
		...
		bool scatter(in ray r, inout hit_record rec, inout vec3 attenuation, inout ray scattered) {
			if (type == 0) {
				...
				return scatterLamCheckered(r, rec, attenuation, scattered);
			}
			else {
				return scatterLam(r, rec, attenuation, scattered);
			}
		...
		bool scatterLamCheckered(in ray r_in, inout hit_record rec, inout vec3 attenuation, inout ray scattered) {
			vec3 target = rec.p + rec.normal + random_in_unit_sphere(rec.uv);
			ray s;
			s.init(rec.p, target - rec.p);
			scattered = s;
			attenuation = checker_texture(rec.p, albedo, vec3(1,1,1));
			return true;
		}
	...
	float dist_to_focus = _FocusDistance;
	float aperture = 0.1f;
	camera cam;
